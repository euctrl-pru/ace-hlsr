...
```{r, echo=FALSE}
## import data
pie_data  <-  read_xlsx("//sky.corp.eurocontrol.int/DFSRoot/Groups/HQ/dgof-pru/Project/ACE/Data for ACE landing page and digital reports/hlsr2021_data.xlsx",
                       sheet = "F_Revenue",
...
trm_reve_reve <- pie_er_trm_data %>%
  filter(TYPE == 'TRM') %>% 
  select(REVE) %>% pull() 

total_reve_reve <- pie_er_trm_data %>%
  summarise(sum(REVE))%>% pull()

start_point_er_trm <- 90+trm_reve_reve/total_reve_reve*180
...
#filter ERT data for pie
pie_er_data <- pie_data_pivot %>% 
  select(ERT, CONCEPT, LABEL,MYCOLOR) 
...
total_er_reve <- pie_er_data %>%
  summarise(sum(ERT))%>% pull()

start_point_er <- 270+(total_er_reve-er_charge_reve)/total_er_reve*180
er_domain_x0 <- 0
er_domain_x1 <- 0.27
...
pie_trm_data <- pie_data_pivot %>% 
  select(TRM, CONCEPT, LABEL,MYCOLOR) 
...
total_trm_reve <- pie_trm_data %>%
  summarise(sum(TRM))%>% pull()

start_point_trm <- 45+(total_trm_reve-trm_charge_reve)/total_trm_reve*180
trm_domain_x0 <- 0.73
trm_domain_x1 <- 1
...
```{r, echo=FALSE, warning=FALSE, out.width="99%", out.height="320px"}
...
myannotations <- list(
  list(
        ...
        text = paste0("<b>", 
                      "Total en-route\nrevenue: ", 
                      "\u20AC ",
                      format(round(total_reve_reve-trm_reve_reve,0), big.mark = " "),
                      " M</b>"),
        xref = "paper",
        yref = "paper",
        xanchor = "center",
    ...
    list(
        ...
        text = paste0("<b>", 
                      "Gate-to-gate\nrevenue: ", 
                      "\u20AC ",
                      format(round(total_reve_reve,0), big.mark = " "),
                      " M</b>"),
        xref = "paper",
        yref = "paper",
        xanchor = "center",
      ...
      list(
        ...
        text = paste0("<b>", 
                      "Total terminal\nrevenue: ", 
                      "\u20AC ",
                      format(round(trm_reve_reve,0), big.mark = " "),
                      " M</b>"),
        xref = "paper",
        yref = "paper",
        xanchor = "center",
...
arrow_left <- image_read(paste0(image_folder,"/dark_blue_left_arrow.svg")) 
